...
export default function BranchesSection() {
  ...
  const handleNavigate = (branch) => {
    const openDirections = (origin) => {
      ...
      const url = `https://www.google.com/maps/dir/?api=1&${originParam}destination=${encodeURIComponent(dest)}&travelmode=driving`
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
          ...
          <div className="md:col-span-1 flex flex-col gap-4">
            {branches.map((b) => (
              <BranchCard key={b.id} branch={b} selected={selected === b.id} onSelect={handleSelect} onNavigate={handleNavigate} />
            ))}
          </div>
        </div>
      </div>
